...
#+begin_src emacs-lisp
  (pretty-hydra-define iensu-hydra
    ...
    ("Email"
     ...
     "Org clock"
     (("c s" org-clock-in                    "start clock")
      ("c r" org-clock-in-last               "resume clock")
      ("c o" org-clock-out                   "stop clock")
...
Add work related org files to refile targets.

#+begin_src emacs-lisp
  (dolist (filename (remove-if-not
                     (lambda (fname)
                       (string-match-p "^work..+.org$" fname))
                     (directory-files iensu-org-dir)))
    (add-to-list 'iensu-org-refile-targets (format "%s/%s" iensu-org-dir filename)))
#+end_src
