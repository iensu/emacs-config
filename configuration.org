...
#+begin_src emacs-lisp :tangle (iensu/tangle-is-feature-enabled 'pinentry)
...
:PROPERTIES:
:header-args: :tangle (iensu/tangle-is-feature-enabled 'gcal-calendar)
:END:
...
** Email
:PROPERTIES:
:header-args: :tangle (iensu/tangle-is-feature-enabled 'email)
:END:
...
Integrate LSP with company mode.
...
Integrate LSP with ivy to search through symbols in a workspace.
...
Add syntax highlighting for NixOS configuration files.
